package com.anthem.ais.pmb.claims.microservices.util;

public interface PMBConstants {

	public final static String RESPONSE_SUCCESS = "SUCCESS";
	public static final String RESPONSE_ERROR = "ERROR";
	public static final String EMPTY_SPACE = "";
	public static final String TIMESTAMP = "yyyy-MM-dd hh:mm:ss.SSS";
	// DateFormats
	public static final String MMMM_DD_YYYY = "MMMM dd,yyyy";
	public static final String MM_DD_YYYY = "MM/dd/yyyy";
	public static final String YYYY_MM_DD = "yyyy-MM-dd";
	// HOST URL
	public static final String EDGE_SERVICE_HOST_URL = "pmb.claims.edgeservice.url";
	public static final String VIRTUAL_HOST_URL = "pmb.claims.virtual.url";
	public static final String SUCCESS_CODE = "PMB_200";
	public static final String AES_ALGO = "aes";
	public static final String API_KEY = "bQFM7EGQlNKqoYAmuTnGJBk1hDGZ6XvB";
	public static final String INSTAMED_URL = "pmb.instamed.pay.request.url";
}
